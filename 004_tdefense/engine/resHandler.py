...
import json
import game.tower
import game.suncatcherTower
import game.laserTower
import game.miningTower
import game.enemy
import game.ore
...
class ResHandler(object):
    ...
    def addPrototype(self, resFile):
        with open(resFile) as f:
            ...
            if type == "suncatcher":
                self.prototypes[type] = game.suncatcherTower.SuncatcherTower(data)
            elif type == "laser":
                self.prototypes[type] = game.laserTower.LaserTower(data)
            elif type == "miner":
                self.prototypes[type] = game.miningTower.MiningTower(data)
            elif type == "base":
                self.prototypes[type] = game.tower.Tower(data)
            elif type == "node":
                self.prototypes[type] = game.tower.Tower(data)
            elif type == "UFO":
                self.prototypes[type] = game.enemy.Enemy(data)
            elif type == "ore_small" or type == "ore_medium" or type == "ore_large":
                self.prototypes[type] = game.ore.Ore(data)
            else:
                print "Unknown prototype", type

    def addAnimation(self, data):
        print self, "Adding animation prototype", data["name"], "with gfx", data["gfx"]
        self.animations[data["name"]] = engine.animation.Animation(data)
